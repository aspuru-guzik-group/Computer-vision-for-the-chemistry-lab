...
def subclass_loss(subclass_logits, sub_labels):
    sub_labels = torch.cat(sub_labels, dim=0).float()
    vessel_flag = torch.sum(sub_labels[:, :6], dim=1) == 0
    loss = F.binary_cross_entropy_with_logits(subclass_logits[vessel_flag], sub_labels[vessel_flag])
    if torch.isnan(loss).any():
        loss = torch.zeros(1).to(subclass_logits.device)
    return loss
...
class RoIHeads(torch.nn.Module):
    ...
    def select_training_samples(self, proposals, targets):
        ...
        gt_sub_labels = [t["sub_cls"] for t in targets]
        ...
        for img_id in range(num_images):
            ...
            matched_gt_sub.append(gt_sub_labels[img_id][matched_idxs[img_id]])

        regression_targets = self.box_coder.encode(matched_gt_boxes, proposals)
        return proposals, matched_idxs, labels, regression_targets, matched_gt_sub
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        if self.training:
            proposals, matched_idxs, labels, regression_targets, sub_labels = self.select_training_samples(proposals, targets)
        else:
            labels = None
        ...
        class_logits, box_regression, sub_cls_logits = self.box_predictor(box_features)
        ...
        if self.training:
            ...
            loss_sub_classifer = subclass_loss(sub_cls_logits, sub_labels)
            losses = {
                "loss_classifier": loss_classifier,
                "loss_box_reg": loss_box_reg,
                "loss_sub_classifer": loss_sub_classifer
            }
        else:
            boxes, scores, labels, sub_labels = self.postprocess_detections(class_logits,sub_cls_logits, box_regression, proposals, image_shapes)
